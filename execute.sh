...
## if multiple terminations are required use the following format
# var_th_ess=(10 20 30) # Minimum average differential variability threshold for ESS (in %)
# window_ess=(250 500 600) # Sliding window size for ESS
## -------------------------------------------------
...
# Execute generate_annotations.py
echo "Generating annotations..."
python generate_annotations.py $termination_criterion $DEBUG $stride $sampling_rate $range_min $range_max $range_tolerance $annotations_output_folder $input_dirs $levs_df $window_levs $stable_th_levs $var_th_esss $window_esss 
...
# Execute create_trajectory_files.py
echo "Generating trajectories..."
python create_trajectory_files.py $termination_criterion $DEBUG $trajectories_output_folder $stride $top_file $input_dirs $annotations_output_folder $first_batch $trim_first_rand $trial $run_last_full
